...
oden.HttpServerRequest = ideal.Observable.clone().newSlots({
	...
	start: function()
	{
		var nodeRequest = this._nodeRequest;
		if (this.encoding())
		{
			nodeRequest.setEncoding(this.encoding());
		}
		var self = this;
		var data = "";
	...
	{
		return Object.perform(this.headers().lowerCased().at('accept-encoding'), "contains", "gzip");
	},
	
	queryAt: function(k)
	{
		return this.queryMap().copy().at(k);
	}
});