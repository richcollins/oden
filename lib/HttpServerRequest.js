...
oden.HttpServerRequest = ideal.Observable.clone().newSlots({
	...
	setNodeRequest: function(nodeRequest)
	{
		...
		nodeRequest.on('end', function(){
			if (nodeRequest.url.contains("creatives.json"))
			{
				console.log("HttpServerRequest receivedBody: " + nodeRequest.url);
				self.observations().forEach(function(o){
					console.log("HttpServerRequest observer: " + o.observer());
				});
